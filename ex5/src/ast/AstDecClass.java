...
public class AstDecClass extends AstNode {
	...
	public Type semantMe() throws SemanticException {
		...
		for (AstFieldList it = fields; it != null; it = it.tail) {
			if (it.head.decVar != null) {
				...
				// We need to validate types exist here because we use them for overriding
				// checks
		...
		for (AstFieldList it = fields; it != null; it = it.tail) {
			if (it.head.decVar != null) {
				...
				method.enclosingClass = classType;
	...
	public temp.Temp irMe() {
		// [1] Process all methods in this class
		for (AstFieldList it = fields; it != null; it = it.tail) {
		...
		// [2] Build vtable for this class
		TypeClass tc = (TypeClass) SymbolTable.getInstance().find(id);
		java.util.List<ir.ClassLayout.MethodEntry> vtable = ir.ClassLayout.getMethodsInOrder(tc);

		java.util.List<String> methodLabels = new java.util.ArrayList<>();
		for (ir.ClassLayout.MethodEntry entry : vtable) {
			methodLabels.add(entry.name);
		}

		ir.Ir.getInstance().AddIrCommand(new ir.IrCommandVTable(id, methodLabels));
